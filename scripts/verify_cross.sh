...
# Usage: scripts/verify_cross.sh [--opencv-test-build-dir DIR] [--auto-build DIR] [--manual-build DIR]
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OPENCV_TEST_BUILD_DIR="$HERE/build/aarch64"
...
echo "== verify_cross $OPENCV_TEST_BUILD_DIR =="
for f in "$AUTO_B/opencv_camera_api_test_cpp" "$MANUAL_B/manual_suite" \
         "$OPENCV_TEST_BUILD_DIR/bin/opencv_test_videoio" "$OPENCV_TEST_BUILD_DIR/lib/libopencv_videoio.so.5.1.0"; do
...
if [[ -z "$QEMU" ]]; then
  ...
  if [[ -x "$OPENCV_TEST_BUILD_DIR/bin/opencv_version" ]]; then
    "$QEMU" -L / "$OPENCV_TEST_BUILD_DIR/bin/opencv_version" && echo "OK smoke: opencv_version" || fail=1
  else
  ...
  "$QEMU" -L / "$OPENCV_TEST_BUILD_DIR/bin/opencv_test_videoio" --gtest_list_tests 2>/dev/null | head -n 3 || true
fi
